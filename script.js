(() => {
  ...
  */
  const 옛재료이름 = {
    /* 🔴 **매장마다 이름표가 다른 재료라 병기를 남긴다**(2026-08-31 사용자님 확정).
       ■ 서초점 이름표는 **「화조유(花椒油)」**다(사진으로 확인). 그런데 **다른 매장에서는 「산초기름」**을
         보셨다 — 그래서 큰 방침(병기를 없앤다)의 **예외**에 해당한다.
       ⚠️ 이 줄을 지우면 「산초기름」으로 저장된 옛 소스의 재료가 「기타」로 떨어진다. */
    '산초기름': '산초기름(화조유)',
    /* 🔴 **병기를 뗐다**(2026-08-31 사용자님 지시). 큰 방침(병기는 장기적으로 없앤다)을 따른 것이다.
       ⚠️ 2026-08-29 에 사용자님이 병기로 확정하셨던 셋 중 하나였다 — **이번에 뗀 것이 확정이다.**
       ⚠️ 남은 병기는 마라시즈닝(고춧가루) · 산고추(고추장아찌) · 산초기름(화조유) 셋이다. */
    '오향우육(다진 고기)': '오향우육',
  };
  ...
  /* ══════════════════════════════════════════════════════════════════════════
     ...
     ■ 무엇: 소스바 재료 42가지를 늘어놓고, 눌러 담고, 양과 단위를 정한다.
  ...
  (function 소스만들기() {
    ...
    /* 🔴 **이 화면도 정식 이름(병기 포함)으로 부른다**(2026-08-29 사용자님 지시로 되돌아왔다).
       ...
       ⚠️ 걸리는 재료는 셋이다 — **마라시즈닝(고춧가루) · 산고추(고추장아찌) · 산초기름(화조유)**.
          ...
          ⚠️ 오향우육은 2026-08-31 에 병기를 뗐고, 산초기름은 같은 날 병기가 붙었다.
    ...
    function 줄HTML(n) {
      ...
      /* 🔴 담기면 **양 조절이 아랫줄로 내려간다**(2026-08-25 사용자님 확정).
         ...
         36개 중 28개가 잘렸다**(2026-08-30 320px 실측. 지금은 42개다 — 「라조장」·「사차장」까지 잘렸다).
