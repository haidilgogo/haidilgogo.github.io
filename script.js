(() => {
  ...
  const GUIDE_BANNER = {
    ...
    body:
      ...
      // 🔴 이 대목은 '소스바가 하이디라오의 강점'이라는 데 무게를 싣고,
      //   소스바 이용이 자율인지 강제인지는 건드리지 않는다(2026-07-28 사용자 지시).
      //   - 소제목은 중립 서술. 옛 '소스바를 이용할 사람 수만큼 선택해요'는 "일행 전체가 아니라
      //     쓸 사람만 고르면 된다"로 읽히는데, 그게 맞는지 확인되지 않아 뺐다. 되살리지 말 것.
      //   - 이용료는 단정하지 않는다. 모든 국내 지점이 같은지 공개 자료로 확인되지 않았고
      //     기획초안 6페이지도 출시 전 확인 항목으로 올려뒀다. 기획초안 소제목
      //     '소스바는 별도 이용료가 있어요'는 단정문이라 쓰지 않고 '붙을 수 있어요'로만 푼다.
      //   - '금액과 기준은 지점에 따라 다를 수 있어요' 문장도 사용자가 빼기로 함. 되살리지 말 것.
      '<h3>소스바 이용 인원도 선택해요</h3>' +
      '<p>탕을 골랐다면, 소스바를 이용할 인원수도 함께 선택해요. 소스바는 이용 인원수에 따라 이용료가 붙을 수 있어요.</p>' +
      '<p>소스바는 하이디라오를 제대로 즐기는 핵심이에요. 다양한 재료를 원하는 대로 조합해 나만의 소스를 만들 수 있고, ' +
