(() => {
  ...
  (function 소스만들기() {
    ...
    function 단계그리기() {
      ...
      /* 🔴 제목은 세 단계가 **「목적어 + 해주세요」 한 꼴**이다(2026-08-29 사용자님 확정).
         ...
         🔴 **새로 만들 때와 고칠 때가 화면에서 갈리지 않는다**(2026-08-29 사용자님 확정).
            제목도, 아래 묻는 말(「이대로 저장할까요?」)도, 버튼(「저장」)도 같다.
    ...
    function 미리보기그리기() {
      ...
      /* 🔴 **새로 만들 때와 고칠 때가 같은 말이다**(2026-08-29 사용자님 확정 — 「전부 통일」).
         ⚠️ 잠깐 「이대로 수정할까요?」로 갈랐다가 되돌린 것이다. 고치러 들어오면 재료·이름·팁·사진이
            **이미 채워져 있어서** 그것 자체가 「고치는 중」이라는 신호이고, 버튼도 둘 다 「저장」이다.
            **다시 가르자고 제안하지 말 것.** */
