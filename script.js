(() => {
  ...
  // 🔴 SAUCE_BAR 배열 순서 = 화면 세 곳의 재료 순서다 — ① 레시피 재료(아래 sortIngs 가 이 순서로 자동 정렬) ·
  //    ② 「나만의 소스 만들기」의 고르는 목록 · ③ 저장된 내 소스의 재료. 자리를 옮기면 셋이 같이 바뀐다.
  //    묶음(그룹)은 2026-09-05 에 사용자님이 다시 나누셨다. **묶음 안은 가나다순** — 새 재료는 알맞은 묶음의 가나다 자리에 넣을 것.
  //    묶음 이름은 화면에 안 나온다(매장에 그런 구분이 없다). 코드 안에서 덩어리로 부르기 위한 것이다.
  //    나누는 기준은 **그 재료의 주된 성격**이다. 뜨는 도구(국자·스푼)는 참고일 뿐 — 굴소스는 국자로 뜨지만 베이스가 아니다(사용자님 판단).
  /* 🔴 **`(건더기만)` 은 재료 이름이 아니다**(2026-08-25 사용자님 확정). 아래 `CHUNKY` 와 `ings` 의
     ...
     **되살리지 말 것.**
     ...
     ⚠️ 이 변경으로 **쑨디2호소스 2종의 재료 표시 순서가 바뀐다** — `청유훠궈소스` 가 3)고기·건더기가 아니라
        제자리인 1)베이스 소스로 돌아가면서 굴소스보다 앞에 온다. 의도한 결과다. (묶음 이름·번호는 2026-09-05 재편 전 것) */
  const SAUCE_BAR = [                       // 소스바 재료 (레시피의 ings 에 쓰는 이름)
    // 1) 베이스 소스 — 「국자가 꽂힌 통」 넷. 소스의 뼈대가 되는 것(참기름 포함 — 사용자님 지정). 이 넷은 가나다순이 아니라 사용자님이 정한 순서다.
    '땅콩참깨소스', '스위트칠리소스', '완자간장소스', '참기름',
    // 2) 매운 장 — 고추가 본체인 장. 매운소고기소스·더우츠유라죠우도 여기(2026-09-05 사용자님 확정). 아래 쌈장 내력은 4) 한국식 장 것이다.
    /* 🔴 **칠리소스**(鮮椒醬)는 2026-08-31 에 넣었다 — 서초점 이름표를 **그대로** 쓴다(사용자님 확정).
       ■ **스위트칠리소스(甜辣醬)와 다른 통이다.** 한자가 다르고 서초점 사진에도 따로 찍혀 있다.
         이쪽은 붉고 씨가 보이는 생고추 장, 그쪽은 갈색을 띤 달콤한 칠리다.
       🔴 **매장마다 한국어 이름표가 다르다** — 「칠리소스」(서초) · 「칠리소스(매운맛)」 · 「칠리소스(중국식)」.
         셋 다 한자는 鮮椒醬 으로 같다. 앞부분이 「칠리소스」로 공통이라 이 이름이면 어디서든 찾을 수 있다.
         ⚠️ 괄호 안은 **매장이 붙인 구분**이지 재료 이름이 아니다. 병기로 넣지 않는다.
       ⚠️ 아직 **어떤 레시피도 쓰지 않는다**.
       🔴 **향라장**(香辣醬)·**쌈장**도 2026-08-31 에 넣었다.
          ■ 향라장은 「향이 나는 매운 장」이다. 마라(麻辣)와 달리 얼얼하지 않고 매콤·고소한 향이 강하다.
            라조장(辣椒醬)·칠리소스(鮮椒醬)와 **한자가 다르다** — 서로 다른 통이다.
          ■ **한국 매장에 있는 것을 사용자님이 확인하셨다**(향라장은 제주점). 사진은 웹 검색 캡쳐라
            서초점 사진에는 없다. 향라장은 이름표가 흐려 **사용자님이 「향라장」으로 짚어 주신 것**이다.
          ⚠️ 둘 다 아직 어떤 레시피도 쓰지 않는다. */
    /* 더우츠유라죠우 — 2026-09-05 까지는 「기름류」에 있었다.
       🔴 **더우츠유라죠우**(豆豉辣椒油)는 2026-08-31 에 넣었다 — 이름표를 **그대로** 쓴다(사용자님 확정).
          ■ 🔴 **박은영소스(마장)의 「라조장」이 이 통이다**(2026-09-02 밤 사용자님 확정). 하이디라오 이름표에 라조장(辣椒醬)은 없고,
            한국에서 라오간마 油辣椒 를 「라조장」이라 부르는 습관을 따른 셰프 표기로 봤다. **한자는 다르다**(豆豉油辣椒 ≠ 辣椒醬) —
            「같은 통」 증명이 아니라 「셰프가 그렇게 불렀을 법하다」 수준의 추정임을 사용자님이 아시고 확정하셨다.
            `SAUCE_BAR` 에서 「라조장」을 뺐다(43 → 42). 라조장은 라이브 레시피 표기였지만 「내 소스」 기능이 배포 전이라 `옛재료이름` 에는 안 적었다.
          ■ 발효콩장(老干风味豆豉)과는 **다른 통**이다. 같은 발효콩이 들어가지만 저쪽은 장, 이쪽은 고추를 기름에 절인 것이다.
            ⚠️ 서초점 사진(`확인필요-더우츠유라죠우.JPG`)상 **건더기가 거의 전부**이고 기름은 그 사이에 조금 고인 정도라 기름류가 아니다 —
            매운 장에 둔다(2026-09-05 사용자님 확정). 도구도 국자가 아니라 작은 스푼이다.
          ■ **통은 서초점 사진에 있다**(`확인필요-더우츠유라죠우.JPG`). 이름은 웹 검색 캡쳐로 읽었다.
          ⚠️ 서초점 사진 한 장이 근거다. 다른 매장에도 있는지, 이름표가 같은지는 아직 모른다.
          ■ 쓰는 레시피는 **박은영소스(마장)** 하나다(위 「라조장」을 이 이름으로 바꾼 것). */
    '고추귀리소스', '더우츠유라죠우', '매운소고기소스', '청유훠궈소스', '칠리갈릭소스', '칠리소스', '향라장',
    // 3) 고소·발효 장 — 안 매운 장. 굴소스(감칠맛 장, 레시피에서 3~5스푼씩 넉넉히 쓴다)·만구향도 여기(2026-09-05 사용자님 확정).
    /* 🔴 **해선장**(海鲜酱 · Seafood sauce)은 2026-09-02 밤에 넣었다 — 사용자님이 쇼츠에서 **이름표를 캡처**해 확인하셨다.
       ■ 이름표 그대로 「해선장」. 병기 없음. 어느 지점인지는 모른다(서초·건대·가산 사진에는 없다 — 향라장처럼 매장마다 다르다).
       ■ 광둥식 해선장(검고 걸쭉·달콤)과 달리 통 안은 **붉은 갈색에 묽고 건더기가 섞여** 있었다. 맛은 미확인.
       ■ 굴소스(蚝油)와 다른 통이다.
       ⚠️ 아직 **어떤 레시피도 쓰지 않는다**(소스 만들기에서만 고를 수 있다). 그림은 2026-09-03 에 넣었다(`해선장.webp`). */
    '굴소스', '만구향', '발효두부소스', '발효콩장', '버섯소스', '부추소스', '사차장', '해선장',
    // 4) 한국식 장 — 한국 매장에만 있는 장. 초장은 초고추장(고추장 베이스)이라 장이 맞다(2026-09-05 사용자님 확정). 쌈장 내력은 2) 위 칠리소스 주석에.
    /* 흰식초·초장 (흰식초는 5) 초류에 있다 — 둘을 같은 날 넣어 내력을 함께 적었다)
       🔴 **흰식초**(白醋)·**초장**은 2026-08-31 에 넣었다 — 이름표를 **그대로** 쓴다.
          ■ 2026-09-02 밤 「흰초」→「흰식초」로 바꿨다(사용자님 지시). 그림 파일도 `흰식초.webp` 로 같이 바꿈. 배포 전이라 `옛재료이름` 에는 안 적었다(8/31 둘과 같은 처리).
          ■ **한국 매장에 있는 것을 사용자님이 확인하셨다**(2026-08-31). 다만 **사진은 웹 검색 캡쳐**라
            어느 지점인지는 적어 두지 못했다 — 서초점 사진(`data/sauce-bar/01_src/`)에는 없다.
          ⚠️ 흰식초는 눌러 쓰는 병에 담겨 있다. 초장은 **초고추장**(광택 있는 적갈색·걸쭉)이다 —
             참고 사진 `data/sauce-bar/reference/초장-웹사진-2026-09-02.jpeg` 로 2026-09-05 확인. 옛 주석의 「맑은 액체」는 틀렸다.
       ⚠️ 둘 다 아직 **어떤 레시피도 쓰지 않는다**. */
    '쌈장', '초장',
    // 5) 초류 (2026-09-05 신설)
    '중국식초', '흰식초',
    // 6) 기름류 — 붓는 기름만. 더우츠유라죠우는 2) 매운 장으로 옮겼다.
    '고추기름', '산초기름(화조유)',
    // 7) 고기·견과류 — 옛 「고기·건더기」. 땅콩가루를 여기로 옮기고 이름을 바꿨다(2026-09-05 사용자님 확정). 참깨는 씨앗이라 넣지 않는다.
    /* 🔴 **오향우육은 병기를 뗐다**(2026-08-31 사용자님 최종 확정).
       ■ 근거는 **매장 이름표**다 — 서초점 사진에 「오향우육 / 五香牛肉」뿐이고 「다진 고기」는 없다.
         **앱에만 있는 설명은 매장에서 그 통을 못 찾게 만든다**(사용자님 말씀).
       ⚠️ 「생소한 말이라 설명이 필요하지 않나」로 **같은 날 두 번 뒤집혔다** — 뗐다 → 붙였다 → 뗐다.
          **이것이 최종이다. 다시 붙이자고 제안하지 말 것.**
       ■ 뜻을 알려 주는 몫은 **재료 사진**이 맡는다(얼개 8절 — 재료 고르는 화면).
       ⚠️ **이름 바뀜표에는 안 적었다**(2026-08-31 사용자님 확정) — **아직 배포 전**이라 이 이름으로
          저장한 사람이 사용자님뿐이고, 시험용 소스라 그냥 두기로 했다. 그래서 옛 이름
          「오향우육(다진 고기)」로 저장된 시험 소스에서는 그 재료가 **「기타」로 내려간다.**
          🔴 **배포 뒤에 이름을 바꿀 때는 반드시 바뀜표에 적어야 한다.** */
    '땅콩가루', '오향우육', '튀긴대두',
    // 8) 채소·향신 — 마라시즈닝(고춧가루)은 말린 고추를 간 향신료라 여기(2026-09-05 사용자님 확정).
    /* ⚠️ **방울토마토는 소스 만들기에서만 안 보인다**(2026-08-31 사용자님 확정 — 아래 `만들기제외`).
       소스바에 실제로 있고 `토마토탕`(b4) 이 쓰므로 **목록에는 그대로 둔다.** */
    /* 🔴 **청양고추**(青阳辣椒)는 2026-09-02 밤에 넣었다 — 사용자님이 매장 전경 사진(어느 지점인지 모름, 겨울)의 이름표로 확인.
       ■ 초록 생고추를 썰어 담은 통. 이름표 그대로, 병기 없음. 태국고추(泰国椒, 빨강) 옆에 둔다.
       ■ 같은 사진의 「빨강청양고추(红青阳辣椒)」는 사용자님이 **무시하기로**(태국고추와 내용물이 비슷해 보이나 한자가 달라 같은 통이라 단정 못 함).
       ■ 같은 사진에 「고추가루(海椒干碎)」·「간장소스(丸滑酱油)」 이름표도 있는데 한자가 같아 앱의 마라시즈닝(고춧가루)·완자간장소스와 같은 통이다 — 매장마다 한글이 다를 뿐.
       ⚠️ 아직 어떤 레시피도 쓰지 않는다. 참고: `data/sauce-bar/reference/소스바-매장전경-이름표-2026-09-02.jpeg`(직원 얼굴 있음 — 앱에 안 씀). */
    '고수', '다진 마늘', '다진 파', '마라시즈닝(고춧가루)', '방울토마토', '산고추(고추장아찌)', '셀러리', '양파', '와사비', '청양고추', '태국고추',
    // 9) 조미 — 마지막에 간을 맞추거나 뿌리는 것. 참깨는 씨앗이라 견과류가 아니고, 뿌리는 것이라 여기(2026-09-05 사용자님 확정).
    '미원', '설탕', '소금', '참깨',
  ];
  const ORDER_ITEMS = ['공깃밥', '날계란', '생면', '만두', '팡가시우메기', '우삼겹', '새우완자', '새우완자/날치알새우완자', '유부', '두유피튀김', '우유/청유 마라훠궈', '청유마라훠궈', '토마토탕훠궈', '버섯탕훠궈', '맑은 탕(맹물)', '거름망'];  // 직원에게 주문하는 항목 (order 에 쓰는 이름)
  ...
  /* ── 재료 **이름 바뀜표** — 2026-08-29 사용자님 지시로 미리 만들어 둔다 ──────────────
     ...
     보기 (지금은 안 쓰지만 꼴을 남겨 둔다):
       ...
       '간장소스': '완자간장소스',            // 매장 이름표를 따를 때
  */
  const 옛재료이름 = {
    /* (비어 있다 — 이름을 바꿀 때 여기에 한 줄씩 적는다)
       🔴 **2026-08-31 에 재료 이름 둘을 바꿨지만 여기 안 적었다**(사용자님 확정) —
          산초기름 → 산초기름(화조유) · 오향우육(다진 고기) → 오향우육 · (2026-09-02) 흰초 → 흰식초.
          **아직 배포 전**이라 그 이름으로 저장한 사람이 사용자님뿐이고, 시험용 소스라 그냥 두기로 했다.
       🔴 **배포 뒤에 이름을 바꿀 때는 반드시 여기에 적어야 한다.** 안 적으면 그 이름으로 저장된
          남의 소스에서 그 재료가 소스바 재료로 안 읽히고 **「기타」로 내려간다.** */
  };
  ...
  const UNITS = ['스푼', '국자', '바퀴', '개', '공기', '그릇', '접시', '인분', '가득', '넉넉하게', '적당히', '조금', '한 꼬집'];
  ...
  const STORES = [
    ...
    // 영등포점은 2026-08 말 이전(경인로 870 2층 → 영중로 28 5층, 타임스퀘어 쪽). 전화도 바뀜(02-2678-0715 → 02-2633-0119, 카카오맵 기준).
    // 네이버·카카오 장소 번호는 그대로라 STORE_*_ID 는 손대지 않았다(2026-09-09 사용자 확정).
    { region: '서울', name: '영등포점',  addr: '서울 영등포구 영중로 28, 5층',                   hours: '10:00 – 03:00', tel: '02-2633-0119' },
  ...
  /* 🔴 **소스 만들기에서만 감추는 재료**(2026-08-31 사용자님 확정).
     ■ 소스바에 **실제로 있는** 재료라 `SAUCE_BAR` 에는 그대로 둔다 — 그래야 레시피가 쓸 수 있고,
       재료 줄 세우기(`sortIngs`)와 검사기도 제자리를 안다.
     ■ 다만 **소스에 넣는 재료가 아니다** — 탕에 넣어 끓이는 것이라 소스 만들기 목록에 있으면 헷갈린다.
       자몽즙·익힌 토마토처럼 **셰프가 쓴 재료**에 가깝다는 것이 사용자님 판단이다.
     ⚠️ 여기 넣으면 **고르지 못할 뿐** 레시피 화면·정렬·검사기에는 그대로 남는다. */
  const 만들기제외 = ['방울토마토'];
  ...
  // 다섯 번째 칸(몇 번 그릇, 2026-09-03)이 있으면 그릇 번호가 먼저다. '2·3' 같은 묶음은 앞 숫자로 센다. 없으면 맨 뒤.
  const 그릇번호 = (it) => (it[4] == null || it[4] === '') ? Infinity : parseInt(String(it[4]), 10);
  const sortIngs = (ings) => (ings || [])
    ...
    .sort((a, b) => {
      ...
      return (그릇번호(a[0]) - 그릇번호(b[0])) || ra - rb || a[1] - b[1];
    })
    .map((x) => x[0]);
  ...
  const RECIPES = [
    ...
    { id: 's5', date: '2025-04-11', cat: '소스', emoji: '🥣', img: 'assets/cards/영지소스_2025.jpg?v=4', imgFit: 'cover', tint: 'linear-gradient(160deg,#F5E6D3,#E8C9A0)', name: '영지소스', ver: '2025', source: 'YouTube 채널십오야', star: true, person: '이영지', notes: ['계량'], desc: '<b>나영석의 보글보글</b> 촬영 중 <b>이영지</b>가 공개한 소스로, <b>나영석</b>이 <u>그룹 활동을 하고 있음에도 불구하고 솔로 가수가 되기 위해 노력하고 있는 느낌의 소스다</u>라고 했다.',
      ings: [['땅콩참깨소스', '2', '스푼'], ['스위트칠리소스', '1.5', '스푼'], ['태국고추', '2', '스푼'], ['마라시즈닝(고춧가루)', '1.5', '스푼'], ['다진 파', '1.5', '스푼'], ['다진 마늘', '1', '스푼'], ['참기름', '2', '바퀴'], ['완자간장소스', '1', '바퀴'], ['오향우육', '1.5', '스푼'], ['참깨', '', '적당히'], ['설탕', '', '한 꼬집'], ['땅콩가루', '', '한 꼬집']],
      steps: [],
      tip: '' },
    { id: 's7', date: '2025-04-11', cat: '소스', emoji: '🥣', img: 'assets/cards/마크소스.jpg?v=3', imgFit: 'cover', tint: 'linear-gradient(160deg,#FDEBD0,#F5C99B)', name: '마크소스', source: 'YouTube 채널십오야', star: true, person: '마크', notes: ['계량'], desc: '<b>나영석의 보글보글</b> 촬영 중 <b>마크</b>가 최초로 공개한 소스로, <b>나영석</b>이 <u>그룹 활동에 최적화된 소스다</u>라고 했다.',
      ings: [['땅콩참깨소스', '2', '스푼'], ['다진 마늘', '1.5', '스푼'], ['양파', '1.5', '스푼'], ['굴소스', '1', '스푼'], ['태국고추', '1', '스푼'], ['완자간장소스', '2', '스푼'], ['오향우육', '', '적당히'], ['다진 파', '', '적당히'], ['땅콩가루', '', '적당히'], ['마라시즈닝(고춧가루)', '', '적당히']],
      steps: [],
      tip: '' },
    ...
    { id: 's20', date: '2026-06-09', cat: '소스', emoji: '🥣', img: 'assets/cards/박은영소스(마장).jpg?v=1', imgFit: 'cover', tint: 'linear-gradient(160deg,#F6DFC8,#ECBE96)', name: '박은영소스', ver: '마장', source: 'YouTube 밥은영', star: true, person: '박은영', heroDesc: '은영 셰프의 시그니처 마장 소스', notes: ['계량'], desc: '유튜브 <b>밥은영</b> 촬영 중 <b>박은영</b> 셰프가 공개한 소스로, <u>핫바리들은 잘 모르는 두 가지 소스(부추 소스, 홍두부 소스)가 들어가야 된다</u>라고 했다.',
      order: [['우유/청유 마라훠궈', '1', '스푼']],
      ings: [['땅콩참깨소스', '3', '스푼'], ['중국식초', '2', '스푼'], ['산초기름(화조유)', '1', '스푼'], ['다진 파', '2', '스푼'], ['다진 마늘', '1', '스푼'], ['땅콩가루', '1', '스푼'], ['태국고추', '1', '스푼'], ['더우츠유라죠우', '1', '스푼'], ['부추소스', '1', '스푼'], ['발효두부소스', '0.33', '스푼']],
      steps: [],
    ...
    { id: 's21', date: '2025-04-26', cat: '소스', emoji: '🥣', img: 'assets/cards/장하오소스.jpg?v=1', imgFit: 'cover', tint: 'linear-gradient(160deg,#F2EEE6,#E2DACB)', name: '장하오소스', source: 'YouTube 엠뚜루마뚜루 : MBC 공식 종합 채널', star: true, person: '장하오', desc: '<b>전지적 참견 시점</b> 촬영 중 <b>제로베이스원</b>의 <b>장하오</b>가 멤버들과 함께 하이디라오에 방문했을 때 공개한 소스이다.',
      ings: [['다진 파', '1', '스푼'], ['양파', '0.5', '스푼'], ['다진 마늘', '0.5', '스푼'], ['발효콩장', '1', '스푼'], ['참기름', '2', '스푼'], ['굴소스', '1', '스푼'], ['완자간장소스', '0.5', '스푼'], ['오향우육', '1', '스푼'], ['발효두부소스', '2', '스푼'], ['버섯소스', '1', '스푼'], ['매운소고기소스', '1', '스푼'], ['태국고추', '', '적당히'], ['고수', '', '적당히'], ['참깨', '', '적당히'], ['중국식초', '', '적당히']],
      steps: [],
      tip: '태국고추, 고수, 참깨, 중국식초는 취향에 맞게 넣기' },
    { id: 's22', date: '2026-05-25', cat: '소스', emoji: '🥣', img: 'assets/cards/수코소스.jpg?v=3', imgFit: 'cover', tint: 'linear-gradient(160deg,#EAF3E0,#CFE3BC)', name: '수코소스', source: 'YouTube sookoh 수코', person: '수코', desc: '유튜버 <b>수코</b>가 혼자 상하이에 있는 하이디라오 와이탄점에 방문해서 공개한 소스이다.',
    ...
    { id: 's24', date: '2025-10-07', cat: '소스', emoji: '🥣', img: 'assets/cards/김풍소스.jpg?v=1', imgFit: 'cover', tint: 'linear-gradient(160deg,#F4E7D6,#E4CBA6)', name: '김풍소스', source: 'YouTube 라꼰즈', star: true, person: '김풍', desc: '<b>김풍</b> 작가가 <b>파김치갱</b> 멤버들과 함께 하이디라오에서 회식을 하며 공개한 소스로, <u>이 소스 천만 소스 간다</u>라며 소개했다.',
      ...
      steps: [],
      tip: '' },
    { id: 's25', date: '2025-02-28', cat: '소스', emoji: '🥣', img: 'assets/cards/우기소스.jpg?v=2', imgFit: 'cover', tint: 'linear-gradient(160deg,#F0EFD9,#D6D4A2)', name: '우기소스', source: 'YouTube 미연zip MIYEON', star: true, person: '우기', notes: ['판독'], desc: '<b>(여자)아이들</b> <b>미연</b>의 유튜브 촬영 중 같은 멤버 <b>우기</b>가 공개한 소스로, <b>미연</b>이 맛을 보더니 <u>그냥 식초인데</u>라고 했다.',
    ...
    { id: 's28', date: '2026-03-19', cat: '소스', emoji: '🥣', img: 'assets/cards/범이소스.jpg', imgFit: 'cover', tint: 'linear-gradient(160deg,#F3E0D2,#DDB394)', name: '범이소스', source: 'YouTube 때때때 TTT', star: true, person: '곽범', desc: '<b>연차없이 어떡행</b> 촬영 중 <b>곽범</b>이 상하이에 있는 하이디라오 와이탄점에 방문해서 <u>건희 나와 범이 간다</u>며 공개한 소스이다.',
      ...
      steps: [],
      tip: '' },
    { id: 's29', date: '2026-06-11', cat: '소스', emoji: '🥣', img: 'assets/cards/예나의 친구소스.jpg?v=1', imgFit: 'cover', tint: 'linear-gradient(160deg,#FADFDC,#F1B5AE)', name: '예나의 친구소스', nameHtml: '예나<span class="name-sub">의 친구</span>소스', source: 'YouTube 제이팍씨 Jay Park', star: true, person: '예나', desc: '<b>박재범</b>의 유튜브 촬영 중 <b>예나</b>가 공개한 소스이다. <b>예나</b>와 같은 고등학교 같은 반 친구가 만든 것으로, <u><b>건희소스</b>가 유행하기 전 어릴 때부터 <b>건희소스</b>와 비슷하게 만들어 먹었다</u>고 한다.',
      ings: [['땅콩참깨소스', '1', '스푼'], ['스위트칠리소스', '2', '스푼'], ['고추기름', '1', '스푼'], ['굴소스', '1', '스푼'], ['양파', '1', '스푼'], ['다진 마늘', '0.5', '스푼'], ['다진 파', '0.5', '스푼'], ['설탕', '', '적당히']],
      steps: [],
      tip: '' },
    { id: 's30', date: '2025-05-08', cat: '소스', emoji: '🥣', img: 'assets/cards/쯔양소스.jpg?v=1', imgFit: 'cover', tint: 'linear-gradient(160deg,#FDE4D6,#F6C4A6)', name: '쯔양소스', source: 'YouTube 쯔양밖정원', star: true, person: '쯔양', desc: '유튜버 <b>쯔양</b>이 본인의 브이로그 채널 <b>쯔양밖정원</b>에서 공개한 특제 소스로, <u>하이디라오를 두 번째 방문했을 때 직원분이 만들어준 소스에 자기가 넣고 싶은 걸 첨가해서 이 맛이 태어났다</u>고 한다.',
    ...
    { id: 's33', date: '2025-09-24', cat: '소스', emoji: '🥣', img: 'assets/cards/킹키가 코로 만든소스.jpg', imgFit: 'cover', tint: 'linear-gradient(160deg,#EDEFF3,#CBD2DC)', name: '킹키가 코로 만든소스', nameHtml: '킹키<span class="name-sub">가 코로 만든</span>소스', source: 'YouTube 권또또 KWONTTOTTO', person: '킹키', desc: '유튜브 <b>또렌드대탐험</b> 촬영 중 <b>킹키</b>가 소스바 재료들의 냄새를 맡아 가며 만든 소스로, <u>코로 만든 소스</u>라고 했다.',
      ings: [['버섯소스', '1', '스푼'], ['참깨', '1', '스푼'], ['참기름', '0.5', '스푼'], ['다진 마늘', '', '조금'], ['오향우육', '2', '스푼'], ['굴소스', '1', '스푼'], ['마라시즈닝(고춧가루)', '1', '스푼'], ['매운소고기소스', '0.5', '스푼']],
      steps: [],
      tip: '' },
    { id: 's34', date: '2025-07-29', cat: '소스', emoji: '🥣', img: 'assets/cards/사인클소스_2025.jpg', imgFit: 'cover', tint: 'linear-gradient(160deg,#6B7799,#4A5470)', name: '사인클소스', ver: '리뉴얼 · 2025', source: 'X @gimsyabeu', person: '사회인 게임클럽', desc: '유튜버 <b>사회인 게임클럽</b>의 횐이인 <b>김샤브</b>가 추천한 <b>사인클소스</b>의 리뉴얼 버전이다.',
      ...
      steps: [],
      tip: '너무 짜면 간장을 0.5스푼만 넣거나 취향에 맞게 조절하기' },
    ...
    { id: 's36', date: '2024-02-13', cat: '소스', emoji: '🥣', img: 'assets/cards/사인클소스(오리지널)_2024.jpg', imgFit: 'cover', tint: 'linear-gradient(160deg,#6B7799,#4A5470)', name: '사인클소스', ver: '오리지널 · 2024', source: 'X @gimsyabeu', person: '사회인 게임클럽', desc: '유튜버 <b>사회인 게임클럽</b>의 횐이인 <b>김샤브</b>가 추천하는 소스로, <b>건희소스</b>와 궁합이 좋다고 한다.',
      ...
      steps: [],
      tip: '' },
    { id: 's37', date: '2026-07-08', cat: '소스', emoji: '🥣', img: 'assets/cards/우승소스.jpg', imgFit: 'cover', tint: 'linear-gradient(160deg,#E8D9C4,#C9AE8C)', name: '우승소스', source: 'YouTube LionsTV', notes: ['계량'], desc: '<b>삼성 라이온즈</b> 우완 투수인 <b>이승현</b> 선수가 삼튜브 촬영 중 공개한 소스로, <u>월남쌈 찍어 먹으면 맛있다</u>고 했다.',
      ings: [['땅콩참깨소스', '1', '스푼'], ['다진 마늘', '0.5', '스푼'], ['다진 파', '1', '스푼'], ['고추기름', '1', '스푼'], ['태국고추', '1', '스푼']],
      steps: [],
      tip: '' },
    { id: 's38', date: '2026-07-08', cat: '소스', emoji: '🥣', img: 'assets/cards/지광소스.jpg', imgFit: 'cover', tint: 'linear-gradient(160deg,#D8C7AE,#B79A73)', name: '지광소스', source: 'YouTube LionsTV', notes: ['계량'], desc: '<b>삼성 라이온즈</b> 투수인 <b>최지광</b> 선수가 삼튜브 촬영 중 공개한 소스로, 고추기름인 줄 알고 잘못 넣은 청유훠궈소스가 킥이 되어서 호평을 받았다.',
      ings: [['양파', '3', '스푼'], ['다진 파', '2', '스푼'], ['다진 마늘', '2', '스푼'], ['청유훠궈소스', '4', '스푼'], ['굴소스', '0.33', '스푼'], ['완자간장소스', '0.5', '스푼']],
      steps: [],
      tip: '' },
    { id: 's39', date: '2026-09-11', cat: '소스', emoji: '🥣', img: 'assets/cards/샤오쥔&양양소스.jpg', imgFit: 'cover', tint: 'linear-gradient(160deg,#C6B39A,#A58D73)', name: '샤오쥔&양양소스', source: 'YouTube 하이슬기 Hi Seulgi', star: true, person: '샤오쥔&양양', notes: ['계량'], desc: '<b>레드벨벳</b> <b>슬기</b>의 유튜브 촬영 중 <b>WayV</b>의 <b>샤오쥔</b>과 <b>양양</b>이 공개한 소스로, 천엽을 젓가락으로 집어 소기름 훠궈에 담갔다 빼는 방식으로 익힌 뒤 소스를 얹어 먹는 방법을 소개했다.',
    ...
    { id: 'b3', cat: '탕', emoji: '🍲', img: 'assets/cards/스키야키탕.jpg?v=3', imgFit: 'cover', tint: 'linear-gradient(160deg,#F3E3CC,#E0C298)', name: '스키야키탕', desc: '맑은 탕에 소스바 재료를 넣어서 셀프로 만드는 스키야키탕이다.',
      ...
      ings: [['완자간장소스', '', '가득', '', 1], ['굴소스', '3', '스푼', '', 2], ['오향우육', '2', '스푼', '', 2], ['다진 마늘', '0.5', '스푼', '', 2], ['다진 파', '0.5', '그릇', '', 2], ['설탕', '2', '스푼', '', 2], ['소금', '3', '스푼', '', 2]],
      steps: [
        '국자를 이용해 맑은 탕의 물을 소스 그릇에 덜어둔다',
        '물이 끓으면 소스바에서 가져온 그릇 2개의 재료를 모두 넣고 계속 끓여준다',   // 2026-09-04 사용자님 확정(그릇 2개 상자와 짝 — 지새기탕과 같은 형식)
    ...
    { id: 'b4', cat: '탕', emoji: '🍲', img: 'assets/cards/토마토탕.jpg?v=3', imgFit: 'cover', tint: 'linear-gradient(160deg,#FFE9E0,#FFC9B8)', name: '토마토탕', desc: '맑은 탕에 소스바 재료를 넣어서 셀프로 만드는 토마토탕이다.',
      ...
      steps: [
        ...
        '청유마라훠궈 국물 1~2국자를 토마토탕에 넣고 섞어준다',   // 2026-09-04 사용자님 확정 — 주문 항목과 짝
      ],
      tip: '굴소스는 취향에 맞게 조절해서 넣기' },
    /* 마라훠궈탕(b5) — 2026-09-05 사용자님 결정: 숨기지 않고 둔다. 출처는 네이버 블로그 ddo-blog(2026-02-17)의 조합 레시피라 `source` 는 적지 않는다(글쓴이도 「여기저기 서치해서 조합」이라 밝힘).
    ...
    { id: 'b5', cat: '탕', emoji: '🍲', img: 'assets/cards/마라훠궈탕.jpg?v=3', imgFit: 'cover', tint: 'linear-gradient(160deg,#F2DDD3,#D9A48F)', name: '마라훠궈탕', desc: '맑은 탕에 소스바 재료를 넣어서 셀프로 만드는 마라훠궈탕이다.',
      ...
      steps: [
        '직원분께 맑은 탕 물의 절반을 빼 달라고 요청한다',
        '소스바 재료는 넘치지 않게 여러 소스 그릇에 나눠서 담아 온다',   // 2026-09-05 사용자님 문장 — 출처 글의 「한번에 다 안 담기니 나눠서 담으셔요」에서
        '물이 끓으면 소스바에서 가져온 재료를 모두 넣고 계속 끓여준다',
      ],
      tip: '' },
    { id: 'b6', date: '2025-02-05', cat: '탕', emoji: '🍲', img: 'assets/cards/혼자햐탕.jpg?v=2', imgFit: 'cover', tint: 'linear-gradient(160deg,#F2DDD3,#D9A48F)', name: '혼자햐탕', source: 'YouTube 혼자햐', desc: '유튜버 <b>혼자햐</b>가 혼자 하이디라오에 가서 맑은 탕에 소스바 재료를 넣어서 셀프로 만드는 마라탕이다.',
      order: [['맑은 탕(맹물)', '', '']],
      /* 🔴 `그릇수`(2026-09-04 사용자님 확정) — 그릇 여러 개에 **똑같이** 나눠 담을 때 쓴다.
         목록은 한 번만 보여 주고 라벨을 「그릇 × N」으로 단다. 양은 **한 그릇 분량**이다
         (원래 땅콩참깨소스 6스푼·나머지 3스푼을 3으로 나눈 값). 그릇마다 내용이 다르면 `ings` 다섯 번째 칸을 쓴다. */
      그릇수: 3,
      ings: [['땅콩참깨소스', '2', '스푼'], ['참기름', '1', '스푼'], ['산초기름(화조유)', '1', '스푼'], ['고추기름', '1', '스푼'], ['청유훠궈소스', '1', '스푼'], ['다진 마늘', '1', '스푼'], ['굴소스', '1', '스푼'], ['완자간장소스', '1', '스푼'], ['참깨', '', '적당히'], ['소금', '', '적당히']],
      steps: [   // 2026-09-05 사용자님 — 스키야키탕의 1·3번(물 덜어 두기·간 맞추기)을 앞뒤에 넣었다
        '국자를 이용해 맑은 탕의 물을 소스 그릇에 덜어둔다',
        '물이 끓으면 소스바에서 가져온 그릇 3개의 재료를 모두 넣고 계속 끓여준다',   // 2026-09-04 사용자님 확정(그릇 3개 상자와 짝)
        '소스 그릇에 덜어둔 물로 취향에 맞게 간을 맞춘다',
      ],
      /* ⚠️ 팁 문장 속의 「산초기름」은 **병기 없이 그대로 둔다**(2026-08-31 사용자님 확정).
         재료 줄에는 이미 「산초기름(화조유)」로 보이고, 문장에까지 괄호를 넣으면 읽기가 나빠진다. */
    ...
    { id: 'b7', date: '2026-06-10', cat: '탕', emoji: '🍲', img: 'assets/cards/지새기탕.jpg?v=1', imgFit: 'cover', tint: 'linear-gradient(160deg,#FBEBD0,#EFCB94)', name: '지새기탕', source: 'YouTube 지새기의 인간극장', desc: '유튜버 <b>지새기</b>가 마라탕을 셀프로 만들기 위해 레시피를 미리 공부까지 해갔으나, 정작 소스바 앞에서 본인 마음대로 조합하다가 맛이 애매해졌다. 이를 가엽게 여긴 <s>천사가</s> 직원이 나타나 만들어준 비법 레시피이다.',
      order: [['맑은 탕(맹물)', '', '']],
      /* 🔴 다섯 번째 칸 = 몇 번 그릇(2026-09-03 사용자님 확정, 안 B). 소스 그릇 3개를 쓴다.
         1번 = 땅콩참깨소스 한 그릇 가득 · 2번 = 땅콩참깨소스 1스푼·산초기름·소금·설탕·굴소스·마라시즈닝 · 3번 = 완자간장소스·참기름·태국고추·다진 마늘·매운소고기소스·청유훠궈소스
         (사용자님이 직접 나눠 주신 것, 2026-09-03). 칸이 있으면 상세 화면이 「N번 그릇」 상자로 나눠 그린다(`renderIngGroups`). 없는 레시피는 전과 똑같다. */
      ings: [['땅콩참깨소스', '', '가득', '', 1], ['땅콩참깨소스', '1', '스푼', '', 2], ['산초기름(화조유)', '6', '스푼', '', 2], ['소금', '2', '스푼', '', 2], ['설탕', '6', '스푼', '', 2], ['굴소스', '1', '스푼', '', 2], ['마라시즈닝(고춧가루)', '1', '스푼', '', 2], ['완자간장소스', '1', '스푼', '', 3], ['참기름', '1', '스푼', '', 3], ['태국고추', '1', '스푼', '', 3], ['다진 마늘', '1', '스푼', '', 3], ['매운소고기소스', '1', '스푼', '', 3], ['청유훠궈소스', '1', '스푼', '', 3]],
      steps: [
        '직원분께 맑은 탕 물의 절반을 빼 달라고 요청한다',
        '물이 끓으면 소스바에서 가져온 그릇 3개의 재료를 모두 넣고 계속 끓여준다',   // 2026-09-03 사용자님 확정(그릇 3개 상자와 짝)
      ],
      tip: '' },
    ...
    { id: 'r2', cat: '히든메뉴', emoji: '🍚', img: 'assets/cards/토마토달걀밥.jpg?v=3', imgFit: 'cover', tint: 'linear-gradient(160deg,#FFE9E0,#FFC9B8)', name: '토마토달걀밥', desc: '녹진하게 끓인 토마토탕에 달걀물을 풀어, 밥에 끼얹어 비벼 먹는 히든 메뉴이다.',
      order: [['토마토탕훠궈', '', ''], ['날계란', '1', '개'], ['공깃밥', '1', '공기']],
      ings: [['참기름', '0.5', '스푼'], ['오향우육', '2', '스푼'], ['다진 파', '2', '스푼']],
      steps: [
    ...
    { id: 'e2', cat: '히든메뉴', emoji: '🥟', img: 'assets/cards/토마토달걀만두.jpg?v=3', imgFit: 'cover', tint: 'linear-gradient(160deg,#FFE9E0,#FFC9B8)', name: '토마토달걀만두', desc: '녹진하게 끓인 토마토탕에 달걀물을 풀고 만두를 익혀 먹는 히든 메뉴이다.',
      order: [['토마토탕훠궈', '', ''], ['날계란', '1', '개'], ['만두', '1', '인분']],
      ings: [['청유훠궈소스', '1', '스푼'], ['오향우육', '2', '스푼'], ['다진 파', '2', '스푼']],
      steps: [
        '토마토탕이 녹진(꾸덕)해질 때까지 충분히 끓여준다',
        '소스바에서 소스를 만든다',
        '날계란에 물을 조금 섞어 풀어준 뒤, 녹진해진 토마토탕 위에 구멍 뚫린 국자를 대고 계란물을 천천히 부어준다',
      ...
      tip: '토마토탕훠궈 국물 안의 토마토를 국자로 으깨주기' },
    { id: 'e1', cat: '히든메뉴', emoji: '🍢', img: 'assets/cards/유부 새우완자.jpg?v=3', imgFit: 'cover', tint: 'linear-gradient(160deg,#F5EFE4,#DCD2C0)', name: '유부 새우완자', desc: '유부에 새우완자를 채워 넣어 국물에 익혀 먹는 히든 메뉴이다.',
      order: [['새우완자/날치알새우완자', '1', '인분'], ['유부', '1', '인분']],   // 2026-09-05 사용자님 — 「또는」 두 줄로. 옛 팁(「날치알 새우완자로 바꾸거나 둘 다 넣기」)은 지웠다
      ings: [],
      steps: [
      ...
      tip: '' },
    { id: 'e3', date: '2026-07-09', cat: '히든메뉴', emoji: '🍢', img: 'assets/cards/유부 새우완자(업그레이드).jpg?v=2', imgFit: 'cover', tint: 'linear-gradient(160deg,#F5EFE4,#DCD2C0)', name: '유부 새우완자', ver: '업그레이드', source: 'YouTube 풍류천재 조서형', desc: '새우완자에 소스바 재료를 첨가하여 유부에 채워 넣고 국물에 익혀 먹는 히든 메뉴이다.',
      order: [['새우완자/날치알새우완자', '1', '인분'], ['유부', '1', '인분']],   // 2026-09-05 사용자님 — 오리지널과 같이 「또는」 두 줄로
    ...
    { id: 'e4', cat: '히든메뉴', emoji: '🌯', img: 'assets/cards/두유피말이.jpg?v=3', imgFit: 'cover', tint: 'linear-gradient(160deg,#FBF0DC,#EBD8B0)', name: '두유피말이', source: 'Instagram @lee._.soom', desc: '두유피튀김을 전골에 담갔다 빼서 고기랑 돌돌돌 말아가지고 전골에 푸우우우욱 익혀서 호록하고 먹는 히든 메뉴이다.',
      ...
      tip: '' },
    { id: 'n2', cat: '히든메뉴', emoji: '🍜', img: 'assets/cards/탄탄면.jpg?v=3', imgFit: 'cover', tint: 'linear-gradient(160deg,#F7E6CE,#E5C08C)', name: '탄탄면', desc: '마라훠궈 국물에 익힌 생면을 특제소스에 비벼 먹는 히든 메뉴이다.',
      order: [['우유/청유 마라훠궈', '', ''], ['생면', '1', '인분']],
      ings: [['땅콩참깨소스', '1', '스푼'], ['버섯소스', '1', '스푼'], ['굴소스', '1', '스푼'], ['오향우육', '1', '스푼'], ['튀긴대두', '1', '스푼'], ['다진 파', '1', '스푼'], ['땅콩가루', '1', '스푼']],
      steps: [
        '소스바에서 소스를 만든다',
      ...
      tip: '' },
    { id: 'n1', cat: '히든메뉴', emoji: '🍜', img: 'assets/cards/토마토에그누들.jpg?v=3', imgFit: 'cover', tint: 'linear-gradient(160deg,#FFECDD,#FFC2A6)', name: '토마토에그누들', desc: '녹진하게 끓인 토마토탕에 달걀물을 풀고 생면을 익혀 먹는 히든 메뉴이다.',
      order: [['토마토탕훠궈', '', ''], ['날계란', '1', '개'], ['생면', '1', '인분']],
      ings: [['오향우육', '2', '스푼'], ['다진 파', '2', '스푼']],
      steps: [
        '토마토탕이 녹진(꾸덕)해질 때까지 충분히 끓여준다',
        '소스바에서 소스를 만든다',
        '날계란에 물을 조금 섞어 풀어준 뒤, 녹진해진 토마토탕 위에 구멍 뚫린 국자를 대고 계란물을 천천히 부어준다',
  ...
  (function validateRecipes() {
    ...
    const issues = [];
    RECIPES.forEach(r => {
      // 다섯 번째 칸(몇 번 그릇, 2026-09-03) — 숫자 또는 '2·3' 같은 묶음만. 한 줄에 있으면 그 레시피 전부에 있어야 한다(빠지면 맨 뒤 상자 없이 떨어진다).
      const 그릇있는줄 = (r.ings || []).filter((i) => i[4] != null && i[4] !== '').length;
      if (그릇있는줄 && 그릇있는줄 !== (r.ings || []).length) issues.push(`그릇 번호가 일부 줄에만 있음(${그릇있는줄}/${r.ings.length})  ·  [${r.name}]`);
      (r.ings || []).forEach(([name, , unit, part, bowl]) => {
        if (bowl != null && bowl !== '' && !/^\d+(·\d+)*$/.test(String(bowl))) issues.push(`그릇 "${bowl}" — 숫자 또는 '2·3' 꼴만  ·  [${r.name}] ${name}`);
  ...
  const SAUCE_IMG_V = 6;   // 6: 2026-09-05 내 소스 기본 그림(방울)을 종지 키운 판으로 교체(사용자님 픽셀메이터 작업). 5: 2026-09-03 재료 그림 9장 종지 통일(7·8차) · 방울토마토 새로. 4: 기본 이미지 둘을 바꿔 끼움
  ...
  // 🔴 기본 좋아요(더미) — 화면 숫자 = BASE_LIKES + 실제 방문자 좋아요.
  //    2026-07-30 도입: 소스 20개 중 16개가 0이라 "사람 없는 커뮤니티"처럼 보였다.
  //    Firebase 콘솔에서 숫자를 직접 고치는 방식은 안 쓴다(모바일에서 지옥이고 버전관리가 안 된다).
  //
  //    🔴 목표는 더미를 결국 0으로 없애는 것이다(2026-09-23 사용자 확정).
  //       한 번에 확 줄이면 티가 나니, 실제 좋아요가 쌓이는 것을 보고 회차마다 조금씩 줄인다.
  //       다음 회차의 시점·기준은 미정 — 자동으로 반복하지 말고 사용자와 정한다.
  //    🔴 새 레시피에는 더미를 넣지 않는다(2026-08-13 사용자 확정, 2026-09-23 재확인).
  //       여기 없는 id는 더미 0이다.
  //
  //    ⚠️ 여기 값은 "더미"이고 화면에 뜨는 숫자가 아니다. 실제 좋아요는 계속 바뀌므로
  //       특정 시점의 순위·표시값을 주석에 박아 두지 않는다 — 필요하면 그때 likes를 읽어 계산한다.
  //    ⚠️ 순서는 byPopular 한 곳이 정한다(화면 숫자 많은 순 → 같으면 날짜 최신순 → 같으면 RECIPES 순서).
  //
  //    감축 기록
  //    - 2026-07-30 도입. 이미 실제 좋아요가 있던 것은 원하는 표시값에서 실제분을 뺀 수로 넣었다.
  //    - 2026-09-23 1차 감축: 더미 5 이상은 −2, 1~4는 −1, 0은 그대로(합계 119 → 81).
  //      e3·e2는 0이 되어 화면에 실제분만 보여도 된다고 사용자가 확인했다.
  const BASE_LIKES = {
    // 소스
    s1: 9,    // 건희소스 오리지널 · 2021
    s2: 8,    // 쑨디2호소스 2022
    ...
    s27: 1,   // 건희소스 리뉴얼 · 2025
    s25: 1,   // 우기소스
    s15: 0,   // 라젤이 아는 동생소스  (사용자가 0으로 지정)
    // 탕 — 레시피 탭의 표시 숫자·인기순 정렬에 사용
    b6: 2,    // 혼자햐탕
    b4: 0,    // 토마토탕
    b7: 0,    // 지새기탕
    b3: 0,    // 스키야키탕
    ...
    // 히든메뉴
    e1: 3,    // 유부 새우완자
    r1: 0,    // 메기살덮밥             (사용자가 "실제 데이터 그대로"로 지정)
    r3: 1,    // 희수국밥
    n1: 1,    // 토마토에그누들
    e3: 0,    // 유부 새우완자 업그레이드
    r2: 0,    // 토마토달걀밥
    e2: 0,    // 토마토달걀만두
    n2: 0     // 탄탄면                (사용자가 0으로 지정)
  };
  ...
  /* 🔴 **기본 이미지가 들어갔다**(2026-09-02 사용자님 확정 — 「다음 차례」 2번).
     ...
     ■ 2026-09-05 — 방울 그림을 **사용자님이 픽셀메이터로 종지를 키운 판**(`_내소스기본-가로.png`)으로 갈았다(배치는 그대로 = 방울이 내 소스 기본).
       ...
       숟가락 손잡이가 위 테두리에서 잘리는 것은 **위 50px 구간을 서서히 투명하게** 해서 풀었다(사용자님 선택 — 30·50·70·100·130 비교 뒤). 변환기 EXTRA 가 `_내소스기본-가로-위흐림.png` 를 읽는다.
  ...
  /* 🔴 주문 항목(`ORDER_ITEMS`)은 **메뉴 탭 그림**(`assets/menu/`)을 쓴다(2026-09-03 사용자님 확정).
     ■ 왜 — 주문 줄이 소스바 재료 줄과 같은 부품이라, 그전엔 소스바에 없는 이름이라고 「직접 입력」 그림이 붙어
       공깃밥·우삼겹 줄에 종지 그림이 나왔다. 메뉴 그림은 같은 화풍·투명 배경 540px 이라 36px 로 줄여도 어울린다.
     ■ 「맑은 탕(맹물)」은 2026-09-23 까지 「맑은 탕/맹물」(옛 병기 슬래시)이었다 — 병기는 괄호라는 2026-08-29 규칙에 맞춰 메뉴 탭 이름과 같게 바꿨다(사용자님 확정). 「거름망」은 그림이 아직 없어 빈자리(코덱스 요청 예정).
     ■ 「우유/청유 마라훠궈」·「새우완자/날치알새우완자」는 아래 `둘중하나` 로 **두 줄**(A · 또는 · B)로 펼친다 — 저장값은 그대로다.
     ⚠️ 메뉴 그림에는 `?v=` 가 없다 — 메뉴 탭과 같은 주소여야 한 번 받은 그림을 다시 쓴다. */
  const ORDER_IMG = {
    '공깃밥': '공깃밥', '날계란': '날계란', '생면': '생면', '만두': '만두', '팡가시우메기': '팡가시우메기',
    '우삼겹': '우삼겹', '새우완자': '새우완자', '날치알새우완자': '날치알새우완자', '유부': '유부', '두유피튀김': '두유피튀김',
    '토마토탕훠궈': '토마토탕훠궈', '버섯탕훠궈': '버섯탕훠궈', '맑은 탕(맹물)': '맑은 탕(맹물)', '거름망': null,
    '우유마라훠궈': '우유마라훠궈', '청유마라훠궈': '청유마라훠궈',
  };
  const 둘중하나 = {
    '우유/청유 마라훠궈': ['우유마라훠궈', '청유마라훠궈'],
    '새우완자/날치알새우완자': ['새우완자', '날치알새우완자'],   // 2026-09-05 사용자님 — 유부 새우완자(오리지널). 팁 문장을 지우고 주문 줄로 옮겼다. 이름은 메뉴 탭 표기(띄어쓰기 없음)
  };
  function 재료그림HTML(name) {
    if (SAUCE_BAR.indexOf(name) >= 0) return '<img src="assets/sauce-bar/' + escHtml(name) + '.webp?v=' + SAUCE_IMG_V + '" alt="" draggable="false" onerror="this.remove()">';
  ...
  /* 🔴 그릇별 묶음(2026-09-03 사용자님 확정 — 「안 B: 그릇마다 상자 따로」). 탕은 소스바 재료를 그릇 여러 개에 나눠 담는다.
     ...
     없으면 `el` 자체를 상자(.ing-box)로 써서 **전과 똑같이** 그린다 — 45개 중 지새기탕 하나만 쓴다(2026-09-03).
     ⚠️ `el` 은 index.html 에서 .ing-box 를 달고 있다. 묶을 때는 그 클래스를 떼고 안에 상자를 만든다 — 안 떼면 상자 속 상자가 된다. */
  function renderIngGroups(el, items, 그릇수) {
    const 묶음 = items.some((i) => i[4] != null && i[4] !== '');
    /* 🔴 `그릇수` 는 **그릇마다 내용이 같을 때**(2026-09-04 혼자햐탕). 목록을 한 번만 그리고 라벨만 「그릇 × N」으로 단다 —
       같은 열 줄을 세 번 반복하면 폰에서 재료만 세 화면이 된다. 그릇마다 내용이 다르면 다섯 번째 칸(`묶음`)을 쓴다. */
    const 한벌 = !묶음 && 그릇수 > 1;
    el.classList.toggle('ing-box', !묶음 && !한벌);
    el.classList.toggle('ing-groups', 묶음 || 한벌);
    if (!묶음 && !한벌) { renderIngList(el, items); return; }
    el.innerHTML = '';
    if (한벌) {
      const lab = document.createElement('div');
      lab.className = 'bowl-lab';
      /* 🔴 「1그릇」이 앞에 오는 이유(2026-09-04 사용자님) — 목록이 **한 그릇 분량**이라는 것까지 말해 준다.
         「그릇 × 3」은 무엇이 3개인지만 말해서 목록이 전체 양인지 한 그릇 양인지 알 수 없다.
         「× N」만 재료 양과 같은 빨강(.bowl-x)으로 띄운다 — 숫자만 우리가 적는다(사용자 입력이 아니다). */
      lab.textContent = '1그릇 ';
      ...
      renderIngList(box, items);
      return;
    }
    const order = [];
    const by = new Map();
    items.forEach((i) => { const k = String(i[4]); if (!by.has(k)) { by.set(k, []); order.push(k); } by.get(k).push(i); });
    order.forEach((k) => {
      const lab = document.createElement('div');
      lab.className = 'bowl-lab';
      lab.textContent = k + '번 그릇';   // 숫자만 우리가 적는다 — 사용자 입력이 아니다
      const box = document.createElement('div');
      box.className = 'ing-box';
      el.appendChild(lab); el.appendChild(box);
      renderIngList(box, by.get(k));
    });
  }
  function renderIngList(el, items) {
    el.innerHTML = '';
    items.forEach((i, idx) => {
      /* 🔴 「우유/청유 마라훠궈」 — **둘 중 아무거나**라는 뜻이라 이름을 하나로 못 바꾼다(다섯 레시피 s17·s18·s20·r1·n2 — 토마토탕 b4 는 2026-09-04 에 청유만으로 바꿈).
         대신 화면에서만 두 줄로 펼치고 사이에 작은 「또는」을 둔다(2026-09-03 사용자님 안). 양·단위는 두 줄 다 같다. */
      if (둘중하나[i[0]]) {
        둘중하나[i[0]].forEach((n, k) => {
          if (k) { const or = document.createElement('div'); or.className = 'ing-or'; or.textContent = '또는'; el.appendChild(or); }
          const r2 = document.createElement('div');
          r2.className = 'ing-row' + (k === 0 ? ' ing-row--pair' : '');
          r2.innerHTML = `
        <span class="ing-pic" aria-hidden="true">${재료그림HTML(n)}</span>
        <span class="ing-name">${escHtml(n)}</span>
      ...
      /* 🔴 옵션(네 번째 칸 「건더기만」)은 **이름 아래 작은 글씨**로 내린다(2026-08-29 사용자님 확정).
         ...
         ⚠️ 옵션이 붙은 줄(`.ing-row--opt`)도 **가운데 맞춤 그대로**다(2026-09-09 사용자님 지적으로 되돌림).
            36px 재료 그림이 줄 높이를 정해서 보통 줄·옵션 줄이 다 49px 이라, 가운데 맞춤이어야 숫자·단위가
            위아래 줄과 같은 세로줄에 온다. 옛 「첫 줄 맞춤」은 그림이 없던 때(40 vs 56px)의 처방이었다(styles.css 주석). */
      const 옵션 = 이름반복 ? '' : (i[3] || '');
      ...
      /* 🔴 **재료 그림 칸(2026-09-02 사용자님 확정).** 소스바 재료면 `assets/sauce-bar/` 그림을 36px 로 붙인다(320px 에서는 30px — styles.css).
         ...
         ■ 공유 이미지(카톡 카드)에도 같은 그림을 그린다(2026-09-03 사용자님 지시, `내소스그림만들기`).
         ■ 이름이 반복되는 줄과 소스바에 없는 재료(내 소스의 직접 입력 「기타」)는 **빈 칸**만 둔다 — 그래야 이름 세로줄이 맞는다.
         ■ 파일이 없는 재료가 생기면 `onerror` 로 img 만 빠지고 빈 칸이 남는다(지금은 43장 전부 있다 — 2026-09-03 완료). 재료 고르는 화면과 같은 방식이다.
      ...
      /* 🔴 소스바에 없는 이름(내 소스의 「기타」 직접 입력)은 **그림 없이 빈 칸**(2026-09-05 사용자님 확정 — 그전엔 공통 그림 `_직접입력.webp`).
         ⚠️ `_직접입력.webp` 는 2026-09-05 에 지웠다(9/3~9/5 에는 「소스 담긴 종지」 그림이었다 — `mineThumbHtml` 주석). 파일 없는 소스바 재료가 생기면 그대로 빈자리(지금은 43장 전부 있다). */
  ...
  function openModal(r) {
    ...
    if (r.ings && r.ings.length > 0) {
      ...
      renderIngGroups(document.getElementById('modalIngs'), sortIngs(r.ings), r.그릇수);
    } else {
  ...
  async function 내소스그림만들기(r) {
    ...
    const 그림칸 = 36 + 12;                       // .ing-pic 36 + .ing-row gap 12 (2026-09-03)
    const 이름최대 = 양오른 - 32 - 12 - 상자안왼 - 그림칸; // .ing-amt min-width 32
    ...
    let y = 공유여백 + 머리높이 + 14;
    if (사진 || 기본그림) y += 안쪽폭 + 16;      // 정사각 사진(또는 기본 그림) + .modal-header 위 여백 16
    ...
    const 재료들 = (r.ings || []).map((i) => {
      const 이름 = 정식재료이름(i[0] || '');
      const 줄 = 줄나누기(자, 이름, 이름최대);
      const 옵션 = i[3] || '';
      return { 이름: 이름, 줄: 줄, 옵션: 옵션, 양: i[1] || '', 단위: i[2] || '',
               높이: Math.max(48, 18 + 줄.length * 20 + (옵션 ? 16 : 0)) };   // 48 = 화면 .ing-row(36px 그림 + 6+6)
    });
    ...
    if (사진 || 기본그림) {
      ...
      if (사진) {
        ...
        // 기본 그림 — 크림 바탕 + 사방 6% 여백 + 통째로(`.hc-mine-thumb` 와 같은 규칙)
        c.fillStyle = '#F5EFEC';                    // --bg-cream
        c.fillRect(공유여백, 커서, 안쪽폭, 안쪽폭);
        const 여백 = Math.round(안쪽폭 * 0.06);
        c.drawImage(기본그림, 공유여백 + 여백, 커서 + 여백, 안쪽폭 - 여백 * 2, 안쪽폭 - 여백 * 2);
      }
      c.restore();
    ...
    재료들.forEach((it, idx) => {
      ...
      // 재료 그림 — 화면 `.ing-pic` 과 같은 36px, 세로 가운데. 못 불러왔으면 자리만 비운다(이름 세로줄 유지)
      const 그림 = 재료그림[it.이름];
      if (그림) c.drawImage(그림, 상자안왼, 가운데 - 18, 36, 36);
      const 글x = 상자안왼 + 그림칸;
      c.fillStyle = '#3A241F';
      c.font = 폰트(15, '500');
      const 글묶음 = it.줄.length * 20 + (it.옵션 ? 16 : 0);
      const 글시작 = 가운데 - 글묶음 / 2 + 2;
      it.줄.forEach((줄, i) => c.fillText(줄, 글x, 글시작 + i * 20));
      if (it.옵션) {                                  // .ing-opt — 12px, .ing-unit 과 같은 색
        c.fillStyle = '#AD8177';
        c.font = 폰트(12, '');
        c.fillText(it.옵션, 글x, 글시작 + it.줄.length * 20 + 2);
      }
      /* 🔴 **양·단위는 첫 줄에 맞춘다** — 이름이 두 줄이어도 숫자가 가운데로 내려앉지 않게.
         ⚠️ 화면 쪽(`.ing-row--opt`)은 2026-09-09 에 가운데 맞춤으로 되돌렸다(36px 그림이 줄 높이를 정하므로).
            이 공유 그림은 그림 칸이 없어 줄 높이가 글줄 수로 정해지니 첫 줄 맞춤을 그대로 둔다. */
  ...
  if (modalReportBtn) modalReportBtn.addEventListener('click', () => {
    ...
    /* 🔴 재료를 **이름·양으로 나눠서도** 넘긴다(2026-09-23 제보 폼 「골라서 넣기」). 폼이 줄로 채울 때 쓴다 —
       합친 글(`ings`)을 거꾸로 쪼개면 「다진 마늘 1」 같은 띄어쓰기 이름에서 틀린다.
       `base` 는 소스바 이름(그림 찾기용), `n` 은 화면 이름(「건더기만」 포함). `ings` 는 옛 폼을 위해 그대로 둔다. */
    const 재료항목 = (r.ings || []).map((i) => ({ n: ingLabel(i), base: i[0], q: i[1] || '' }));
    let 담았나 = false;
    try {
      sessionStorage.setItem(REPORT_PREFILL_KEY, JSON.stringify({
        name: r.name || '', ings: 재료줄, items: 재료항목, tip: r.tip || '',
      }));
      담았나 = true;
  ...
  function applyPayload(p) {
    ...
    if (p.mySauces) {
      ...
      /* 🔴 **다른 기기에서 지운 소스의 사진도 여기서 지운다**(2026-09-02 전체 검사에서 잡음).
         이 기기에서 지울 때는 `내소스지우기` 가 사진을 같이 지우는데, 동기화로 들어온 삭제에는
         그 처리가 없었다 — 소스는 목록에서 사라지는데 **사진만 이 기기 저장 공간에 계속 남았다.**
         사진은 서버로 안 가므로 여기 말고는 지울 자리가 없다.
         ■ `deleted` 만 본다 — 합치기가 지운 id 를 반드시 거기에 남기므로 그것이 기준이다.
         ■ `saveMySaucePhotos()` 는 서버로 안 올린다(그 주석 참고). 그래서 이 자리에서 불러도
           **받기 → 올리기** 고리가 안 생긴다. `saveMySauces()` 와는 다르다.
         ⚠️ 지울 것이 있을 때만 저장한다 — 되울림마다 저장소를 다시 쓰지 않으려는 것이다. */
      let 사진지운게있다 = false;
      mySauceData.deleted.forEach((id) => {
        if (id in mySaucePhotos) { delete mySaucePhotos[id]; 사진지운게있다 = true; }
      });
      if (사진지운게있다) saveMySaucePhotos();
  ...
  /* ══════════════════════════════════════════════════════════════════════════
     ...
     ■ 무엇: 소스바 재료 42가지를 늘어놓고(목록은 43가지인데 방울토마토 하나가 빠진다 — `만들기제외`), 눌러 담고, 양과 단위를 정한다.
     ...
     🔴 「건더기만」 칩은 **넣지 않는다**(사용자님 확정). 필요하면 팁에 적는다.
        ...
        이 화면을 위한 것이 아니다.
     ■ 재료 그림 자리(`.si-thumb`)는 2026-09-02 에 넣었고 43장 전부 채워져 있다(2026-09-03).
     ══════════════════════════════════════════════════════════════════════════ */
  (function 소스만들기() {
    ...
    /* 🔴 **이 화면도 정식 이름(병기 포함)으로 부른다**(2026-08-29 사용자님 지시로 되돌아왔다).
       ...
       ⚠️ 걸리는 재료는 셋이다 — **마라시즈닝(고춧가루) · 산고추(고추장아찌) · 산초기름(화조유)**.
          320px 에서 이름이 두 줄로 감기는데, 줄 높이에 자리가 있어 안 깨진다
          (2026-08-25 실측 주석이 `.si-name` 에 있다).
          ⚠️ 산초기름은 2026-08-31 에 병기가 붙었고, 오향우육은 같은 날 뗐다.
    ...
    function 보이는재료() {
      ...
      return 고를수있는재료.filter((n) => n.indexOf(q) >= 0);
    }
    ...
    function 줄HTML(n) {
      ...
      if (!it) {
        ...
        return thumb + '<div class="si-body">' + name
          + '<div class="si-below"><button class="si-add" type="button" aria-label="' + esc(부를이름) + ' 담기"' + (가득찼나() ? ' disabled' : '') + '>' + 더하기아이콘 + '</button></div>'
          + '</div>';
      }
      /* 🔴 알약에 **「스푼」 글자를 안 보인다**(2026-09-23 사용자님 확정 — 담을 때만. 저장한 내 소스 상세·레시피 상세는 「2스푼」 그대로).
         단위가 하나뿐이라 고를 것이 없고, 숫자만으로 뜻이 통한다. 저장값 `u`(기본단위)와 낭독기 이름 「양(스푼)」은 그대로 둔다.
         ⚠️ 예전엔 `.si-unit-txt`(aria-hidden) 로 「스푼」을 그렸다 — CSS 는 남아 있지만 이 화면에서는 더 안 쓴다. */
      /* 🔴 담기면 **양 조절이 아랫줄로 내려간다**(2026-08-25 사용자님 확정).
         한 줄에 그림·이름·조절부 셋을 나란히 두면 **좁은 폰에서 이름 자리가 37px 밖에 안 남아
         36개 중 28개가 잘렸다**(2026-08-30 320px 실측. 지금은 42개다 — 「사차장」까지 잘렸다. 2026-09-02 해선장·청양고추 추가, 라조장 뺌).
    ...
    /* 🔴 줄을 갈아 끼울 때 **초점을 이어 준다**(2026-09-23 코덱스 지적 — 키보드로 담으면 초점이 페이지 맨 위(BODY)로 빠졌다).
       눌렀던 단추가 새 줄에 없어지므로, 같은 역할의 자리로 옮긴다:
       담기(+) → 새 줄의 숫자 칸 · 빼기(✕) → 새 줄의 담기(+) · −/+ → 같은 쪽 단추 · 숫자 칸 → 숫자 칸.
       ⚠️ 초점이 그 줄 **안에 있었을 때만** 옮긴다. 손가락으로 누를 때도 같지만 화면은 그대로라 티가 안 난다. */
    /* ⚠️ `a` 는 **갈아 끼우기 전에** 잡아 둔 초점이다 — 옛 줄을 빼는 순간 초점이 BODY 로 바뀌어 버린다(처음에 그걸 놓쳤다). */
    /* 담기 단추가 잠겨 있을 때(30개 이상) 초점을 받을 **가까운 단추** — 아래 줄부터, 없으면 위 줄에서 찾는다.
       🔴 글자 칸은 고르지 않는다(단추만). 2026-09-24 코덱스 재검증: 30개 넘게 저장된 소스에서 빼고도 30개 이상이면
          잠긴 담기(+)나 초점을 못 받는 이름 글자를 골라 초점이 BODY 로 빠졌다. */
    function 가까운단추(줄) {
      const 고르기 = (r) => r && r.classList && r.classList.contains('si-row')
        && r.querySelector('.si-del, .si-add:not(:disabled)');
      for (let r = 줄 && 줄.nextElementSibling; r; r = r.nextElementSibling) { const b = 고르기(r); if (b) return b; }
      for (let r = 줄 && 줄.previousElementSibling; r; r = r.previousElementSibling) { const b = 고르기(r); if (b) return b; }
      return null;
    }
    function 초점잇기(old, 새줄, a) {
      if (!old || !a || !old.contains(a)) return;
      let 갈곳 = null;
      /* 🔴 담기(+) 뒤에는 숫자 칸이 아니라 **✕ 단추**로 — 글자 칸에 초점이 가면 아이폰이 자판을 올린다(2026-09-24). */
      if (a.classList.contains('si-add')) 갈곳 = 새줄.querySelector('.si-del');
      else if (a.classList.contains('si-del')) 갈곳 = 새줄.querySelector('.si-add:not(:disabled)') || 가까운단추(새줄);
      else if (a.dataset && a.dataset.step) 갈곳 = 새줄.querySelector('[data-step="' + a.dataset.step + '"]');
      else if (a.classList.contains('si-amt')) 갈곳 = 새줄.querySelector('.si-amt');
      else if (a.classList.contains('si-etc-name')) 갈곳 = 새줄.querySelector('.si-etc-name');
      if (갈곳) 갈곳.focus({ preventScroll: true });
    }
    function 줄갱신(n) {
      const old = listEl.querySelector('.si-row[data-n="' + (window.CSS && CSS.escape ? CSS.escape(n) : n) + '"]');
      if (old) { const a = document.activeElement; const 새줄 = 줄만들기(n); old.replaceWith(새줄); 초점잇기(old, 새줄, a); }
      발밑갱신();
    }
    function render() {
      const names = 보이는재료();
      ...
      /* 🔴 「기타」는 **목록 맨 뒤**다(얼개 4절). 지점마다 소스바 구성이 다르고, 탕 국물처럼
         ...
         ⚠️ **검색 중에는 감춘다.** 검색은 소스바 42가지를 좁히는 것이라 직접 적은 것과 섞이면
    ...
    /* 🔴 **재료는 30개까지**(2026-09-23 사용자님 확정 — 제보 폼 「목록에서 고르기」의 재료 30개와 같다).
       ■ 30개가 차면 **더 담는 것만** 막는다: 담기(+)·「재료 직접 입력하기」·빈 기타 칸. 양 조절·빼기(✕)·저장은 그대로.
       ■ 하나를 빼면 다시 담을 수 있다. 검색으로 가려진 재료도 센다(`담은개수` 가 목록 전체를 센다).
       ■ 이미 30개 넘게 저장된 소스(제한 전에 만든 것)는 「수정하기」로 열면 **전부 보이고 저장도 된다** — 더 담기만 막힌다.
         그래서 제보 폼은 30개 넘게 넘어오면 「직접 쓰기」로 전부 받는다(report.html). */
    const 최대재료 = 30;
    const 가득찼나 = () => 담은개수() >= 최대재료;
    /* 다 찼는지에 맞춰 담는 단추들을 켜고 끈다 — 개수가 바뀔 때마다(`발밑갱신`) 부른다.
       🔴 **빈 기타 칸은 읽기 전용**으로 돌린다 — 단추만 막으면 이미 만들어 둔 빈 칸에 이름을 적어 31개가 된다(코덱스 지적).
          이미 이름이 있는 칸은 고칠 수 있다. 치는 도중인 칸(초점)은 건드리지 않는다 — 조합 중인 한글이 깨진다. */
    function 담기단추갱신() {
      const 가득 = 가득찼나();
      listEl.querySelectorAll('.si-add').forEach((b) => { b.disabled = 가득; });
      const 직접 = listEl.querySelector('.si-etc-add');
      if (직접) 직접.disabled = 가득;
      listEl.querySelectorAll('.si-etc-name').forEach((칸) => {
        if (칸 === document.activeElement) return;
        칸.readOnly = 가득 && !칸.value.trim();
      });
    }
    ...
    function 발밑갱신() {
      ...
      if (n) countEl.innerHTML = '<span><b class="sauce-count-n">' + n + '</b>가지 담음</span>'
        /* 🔴 다 차면 개수는 그대로 두고 **아랫줄에 덧붙인다**(2026-09-23 코덱스 권고 채택 — 몇 개 담았는지가 계속 보여야 한다) */
        + (n >= 최대재료 ? '<span class="sauce-limit" role="status">재료는 ' + 최대재료 + '개까지 담을 수 있어요.</span>' : '');
      else countEl.innerHTML = 담기안내;
      countEl.classList.toggle('is-on', n > 0);
      countEl.classList.toggle('is-full', n >= 최대재료);
      담기단추갱신();
    ...
    function 줄다시그리기(row) {
      const it = 항목찾기(row);
      if (row.dataset.etc) {
        if (it) { const a = document.activeElement; const 새줄 = 기타줄만들기(it); row.replaceWith(새줄); 초점잇기(row, 새줄, a); }
        발밑갱신();
        return;
      }
      줄갱신(row.dataset.n);
    ...
    function 빼기(row) {
      if (row.dataset.etc) {
        ...
        const 초점있었나 = row.contains(document.activeElement);
        const 이웃 = 초점있었나 ? 가까운단추(row) : null;   // 빼기 **전에** 찾아 둔다(빼면 이웃을 못 찾는다)
        row.remove();
        발밑갱신();
        /* 키보드로 뺐으면 초점을 「재료 직접 입력하기」로 — 줄이 통째로 사라져 초점이 페이지 맨 위로 빠지지 않게(2026-09-23).
           🔴 그 단추가 잠겼으면(아직 30개 이상) 가까운 줄의 단추로(2026-09-24 코덱스 재검증). */
        if (초점있었나) {
          const 직접 = listEl.querySelector('.si-etc-add:not(:disabled)');
          const 갈곳 = 직접 || 이웃;
          if (갈곳) 갈곳.focus({ preventScroll: true });
        }
        return;
...
(function () {
  ...
  // 🔴 같은 하위 안의 순서 = `o`(2026-09-20 사용자 확정) — 공식 사전주문 13곳의 진열 순서를 **맞대결 우선**으로 합친 순서다
  //    (두 메뉴를 같이 파는 지점들에서 앞선 쪽이 반드시 위. 가위바위보처럼 물고 물리는 묶음 안에서만 표 차이로 결정.
  //    make_menu_cats.mjs 「다수결 순서」). 지점마다 순서가 달라서 어느 한 매장의 태블릿과 똑같지는 않다.
  //    신메뉴가 위인 것은 모든 지점이 같다.
  //    순번이 없는 메뉴(API 에서 사라진 것)는 원래 줄 순서대로 맨 뒤.
  //    sort 는 안정 정렬이라 같은 값끼리는 순서를 지킨다.
  //    🔴 `o`가 정확히 같을 때만 2차 기준: 공식 영구 분류에서 온 순번이 홍보 분류에서 빌린 순번(`ob`)보다 앞
  //    (2026-09-20 사용자 확정 B안 — 특제양고기(추천메뉴 1번)와 부채살(소양돈고기 1번)이 같은 값이라 문서 줄 순서로
  //    특제양고기가 맨 앞이던 것을 바로잡음). 값이 다르면 빌린 순번이 앞에 올 수도 있다 — 다른 목록 값이라 공식에 없는 관계.
  const 하위메뉴 = (up, sub) => ALL.filter((it) => it.up === up && it.sub === sub)
    .sort((a, b) => ((a.o ?? 1e9) - (b.o ?? 1e9)) || ((a.ob ? 1 : 0) - (b.ob ? 1 : 0)));
