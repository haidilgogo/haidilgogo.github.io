(() => {
  ...
  (function 소스만들기() {
    ...
    /* 🔴 이 화면에서는 **매장 이름표 그대로** 부른다(2026-08-25 사용자님 확정).
       ...
       (표기 규칙: 「정식명이 낯설 때 괄호로 붙이는 고정 부연」. 🔴 2026-08-29 에 슬래시에서
       괄호로 바뀌었다 — 「건더기만」을 이름 아래로 내려 재료 줄에서 괄호가 비었기 때문이다),
       소스바 앞에서는
    ...
    /* 🔴 **상태로 말한다**(2026-08-29 사용자님이 잡으심). 예전에는 「`+` 를 눌러 재료를 담아 주세요」라는
       **안내**였는데, 이 줄이 발밑에서 **제목 바로 아래**로 올라오면서 제목과 같은 말을 두 번 하게 됐다.
       ■ 이 자리는 원래 **「지금 몇 개 담았나」**를 말하는 자리다. 담기 전에도 상태로 말하는 것이 앞뒤가 맞다.
       ■ `+` 를 어떻게 누르는지는 **목록의 모든 줄에 `+` 동그라미가 이미 보여 준다.**
       ⚠️ 예전 안내문은 `+` 를 **화면의 그 버튼 모양 그대로** 그렸다(글자로 쓰면 320px 에서 자리를 넘쳤다).
          그 그림 조각(`.sauce-count-ic`)은 남아 있다 — 다시 쓸 일이 생기면 그대로 쓸 수 있다. */
    const 담기안내 = '아직 담은 재료가 없어요';
    ...
    /* 담은 것을 **저장할 꼴 그대로** 만든다 — `[이름, 양, 단위]` 의 배열이다.
       ...
       🔴 이름은 **원래 이름(`SAUCE_BAR`)** 으로 적는다 — 화면에서 짧게 부르는 이름(`표시이름`)으로
          적으면 레시피 데이터와 어긋나 재료 정렬과 검사기가 깨진다(위 `소스바이름` 주석).
