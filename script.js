(() => {
  ...
  const SAUCE_BAR = [                       // 소스바 재료 (레시피의 ings 에 쓰는 이름)
    ...
    // 3) 고기·건더기
    /* 🔴 **오향우육은 병기를 남긴다**(2026-08-31 사용자님 확정).
       ■ **이름표에는 병기가 없다**(서초점 사진 — 「오향우육 / 五香牛肉」뿐이다). 그런데도 남기는 이유는
         **한국 사람에게 생소한 말**이라서다. 「다진 고기」가 그게 무엇인지 알려 주는 유일한 설명이다.
       ⚠️ 2026-08-31 에 큰 방침(병기를 없앤다)을 따라 한 번 뗐다가 **같은 날 되돌렸다.**
          다시 떼자고 제안하지 말 것. */
  ...
  */
  ...
  (function 소스만들기() {
    ...
    /* 🔴 **이 화면도 정식 이름(병기 포함)으로 부른다**(2026-08-29 사용자님 지시로 되돌아왔다).
       ...
       ⚠️ 걸리는 재료는 넷이다 — **마라시즈닝(고춧가루) · 오향우육(다진 고기) · 산고추(고추장아찌) ·
          산초기름(화조유)**. 320px 에서 이름이 두 줄로 감기는데, 줄 높이에 자리가 있어 안 깨진다
          (2026-08-25 실측 주석이 `.si-name` 에 있다).
          ⚠️ 산초기름은 2026-08-31 에 병기가 붙었다.
