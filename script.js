(() => {
  ...
  const SAUCE_BAR = [                       // 소스바 재료 (레시피의 ings 에 쓰는 이름)
    ...
    '매운소고기소스', '오향우육', '튀긴대두',
    // 4) 채소·향신
    /* ⚠️ **방울토마토는 목록에 넣지 않는다**(2026-08-31 사용자님 확정).
       ■ 소스바에 실제로 있고 `토마토탕`(b4) 레시피가 쓰지만, **소스에 넣는 재료가 아니다** —
         탕에 넣어 끓이는 것이다. 자몽즙·익힌 토마토처럼 **셰프가 쓴 재료**에 가깝다.
       ■ 목록에서 빠져도 그 레시피 화면은 그대로 뜬다 — `SAUCE_BAR` 는 소스 만들기와 제보 폼에서만 쓴다.
       ⚠️ 자몽즙·흑초·해선간장·익힌 토마토도 같은 이유로 아직 안 넣었다(레시피를 넣을 때 함께 정한다). */
  ...
  */
  ...
  /* ══════════════════════════════════════════════════════════════════════════
     ...
     ■ 무엇: 소스바 재료 41가지를 늘어놓고, 눌러 담고, 양과 단위를 정한다.
