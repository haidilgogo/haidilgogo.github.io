(() => {
  ...
  // 🔴 지점별 네이버 장소번호 — 지도에 **핀을 찍으려면** 이 번호가 있어야 한다(2026-08-04).
  //    이름만으로 만든 주소(…/search?query=…&mapMode=0)는 검색 결과 목록이나 아무것도 안 찍힌
  //    지도만 뜬다. 뒤에 #map/<번호> 를 붙여야 그 매장에 핀이 박힌다.
  //    ⚠️ 이 형태는 사용자가 실기기에서 검색 결과의 「지도」 버튼을 눌러 얻은 실제 주소다.
      ...
      if (!s.soon || s.addr) {
        ...
        if (s.addr && catchUrl === 'soon') {
          ...
          [
            ...
            //    🔴 뒤의 #map/<장소번호> 가 핀을 찍는다 — 이게 없으면 검색 결과 목록이나
            //       아무것도 안 찍힌 지도만 뜬다(2026-08-04 실기기 확인). 번호는 STORE_NAVER_ID 에 있다.
            //       번호가 없는 지점은 핀 없이 검색 결과로 뜬다 — 안 열리는 것보다는 낫다.
            { label: '네이버지도', img: 'assets/icons/navermap.png?v=1',
              href: 'https://m.map.naver.com/search?query=' + qPlus + '&mapMode=0' +
                    (STORE_NAVER_ID[s.name] ? '#map/' + STORE_NAVER_ID[s.name] : '') },
