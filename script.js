(() => {
  ...
  const SAUCE_BAR = [                       // 소스바 재료 (레시피의 ings 에 쓰는 이름)
    ...
    /* 🔴 **해선장**(海鲜酱 · Seafood sauce)은 2026-09-02 밤에 넣었다 — 사용자님이 쇼츠에서 **이름표를 캡처**해 확인하셨다.
       ...
       ■ 굴소스(蚝油)와 다른 통이다. 쌈장 뒤에 둔 것은 콩 발효 장 계열이라서일 뿐, 서빙 순서는 모른다.
       ⚠️ 아직 **어떤 레시피도 쓰지 않는다**. 그림도 없다(사진 대기). */
    '땅콩참깨소스', '스위트칠리소스', '칠리소스', '고추귀리소스', '칠리갈릭소스', '향라장', '부추소스', '버섯소스', '사차장', '발효콩장', '발효두부소스', '쌈장', '해선장', '청유훠궈소스', '참기름',
    /* 2) 액체 양념
       🔴 **흰식초**(白醋)·**초장**은 2026-08-31 에 넣었다 — 이름표를 **그대로** 쓴다.
          ...
          ⚠️ 흰식초는 눌러 쓰는 병에 담겨 있고, 초장은 짙은 붉은색 맑은 액체라 중국식초와 비슷해 보인다.
       ⚠️ 둘 다 아직 **어떤 레시피도 쓰지 않는다**. */
    '굴소스', '완자간장소스', '중국식초', '흰식초', '초장',
    // 3) 고기·건더기
    ...
    '다진 마늘', '다진 파', '양파', '방울토마토', '태국고추', '청양고추', '산고추(고추장아찌)', '셀러리', '고수', '와사비',
    // 5) 가루류
    '땅콩가루', '마라시즈닝(고춧가루)', '참깨',
    /* 6) 기름류
       🔴 **더우츠유라죠우**(豆豉辣椒油)는 2026-08-31 에 넣었다 — 이름표를 **그대로** 쓴다(사용자님 확정).
          ...
          ■ 발효콩장(老干风味豆豉)과는 **다른 통**이다. 같은 발효콩이 들어가지만 하나는 장, 이쪽은 기름이다.
          ■ **통은 서초점 사진에 있다**(`확인필요-더우츠유라죠우.JPG`). 이름은 웹 검색 캡쳐로 읽었다.
          ⚠️ 서초점 사진 한 장이 근거다. 다른 매장에도 있는지, 이름표가 같은지는 아직 모른다.
          ⚠️ 아직 **어떤 레시피도 쓰지 않는다**(고추귀리소스·사차장·미원과 같다). */
    '고추기름', '산초기름(화조유)', '더우츠유라죠우',
    // 7) 간·기타
    '설탕', '소금', '미원', '만구향',
  ];
  const ORDER_ITEMS = ['공깃밥', '날계란', '생면', '만두', '팡가시우메기', '우삼겹', '새우완자', '유부', '두유피튀김', '우유/청유 마라훠궈', '청유마라훠궈', '토마토탕훠궈', '버섯탕훠궈', '맑은 탕/맹물', '거름망'];  // 직원에게 주문하는 항목 (order 에 쓰는 이름)
