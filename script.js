(() => {
  ...
  const GUIDE_BANNER = {
    ...
    body:
      ...
      '<h3>소스바를 이용할 사람 수만큼 선택해요</h3>' +
      // 🔴 이용료를 단정하지 않는다(2026-07-28 사용자 지시). 모든 국내 지점이 같은지 공개 자료로
      // 확인되지 않았고 기획초안 6페이지도 출시 전 확인 항목으로 올려뒀다. 기획초안의 소제목
      // '소스바는 별도 이용료가 있어요'는 단정문이라 쓰지 않고, 소제목은 독자가 할 행동으로 두고
      // 이용료는 본문에서 '붙을 수 있어요'로만 푼다.
      // ※ '금액과 기준은 지점에 따라 다를 수 있으니 태블릿 안내를 확인해주세요' 문장은 사용자가
      //   빼기로 함(2026-07-28). 되살리지 말 것 — '붙을 수 있어요'만으로 이미 단정을 피한다.
      '<p>탕을 골랐다면, 소스바를 이용할 인원수도 함께 선택해요. 소스바는 이용 인원수에 따라 이용료가 붙을 수 있어요.</p>' +
      '<p>소스바에서는 다양한 재료를 조합해 탕과 고기에 곁들일 소스를 만들 수 있어요. ' +
      '무엇을 넣어야 할지 모르겠다면 하딜고고에서 취향에 맞는 레시피를 찾아보세요!</p>',
  };
