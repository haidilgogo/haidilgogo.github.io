(() => {
  ...
  const SAUCE_BAR = [                       // 소스바 재료 (레시피의 ings 에 쓰는 이름)
    ...
    /* 🔴 **해선장**(海鲜酱 · Seafood sauce)은 2026-09-02 밤에 넣었다 — 사용자님이 쇼츠에서 **이름표를 캡처**해 확인하셨다.
       ...
       ■ 굴소스(蚝油)와 다른 통이다.
       ⚠️ 아직 **어떤 레시피도 쓰지 않는다**. 그림도 없다(사진 대기). */
  ...
  */
  ...
  /* 🔴 **기본 이미지가 들어갔다**(2026-09-02 사용자님 확정 — 「다음 차례」 2번).
     `assets/sauce-bar/_내소스기본.webp`(1080) — 소스가 담긴 흰 종지에 숟가락이 꽂힌 그림. 코덱스가 소스바 41장과 같은 화풍으로 그렸다.
  ...
  function renderIngList(el, items) {
    el.innerHTML = '';
    items.forEach((i, idx) => {
      /* 🔴 「우유/청유 마라훠궈」 — **둘 중 아무거나**라는 뜻이라 이름을 하나로 못 바꾼다(여섯 레시피).
