  ...
  (function 소스만들기() {
    ...
    function 다음단계() {
      ...
      /* 🔴 **이름이 빈 기타 줄은 여기서 버린다.** 눌러 놓고 안 적은 칸인데, 그대로 두면
         「뒤로」로 돌아왔을 때 빈 줄이 남아 있고 저장 자료에도 이름 없는 재료가 섞인다.
         ⚠️ 세지도 않던 줄이라 개수가 달라지지 않는다 — 눈에 띄는 변화는 빈 줄이 없어지는 것뿐이다. */
      for (let i = 기타.length - 1; i >= 0; i--) if (!기타[i].n.trim()) 기타.splice(i, 1);
      단계 = 2;
      nameInput.value = '';
      tipInput.value = '';
      단계그리기();
