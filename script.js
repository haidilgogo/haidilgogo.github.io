(() => {
  ...
  const GUIDE_BANNER = {
    ...
    // 부제는 nowrap이라 길면 …로 잘린다. '셀프'를 붙인 뒤 실측: 214px / 자리 244px → 여유 30px로 안전.
    // 같은 문구가 index.html의 홈 가이드 배너(.home-guide-sub)에도 있다 — 한쪽만 고치지 말 것.
    heroDesc: '주문부터 셀프소스바 사용까지 · 가이드',
    ...
    body:
      ...
      // alt의 '02 소스바에서 소스 만들기'는 본문을 '셀프소스바'로 바꾼 뒤에도 그대로 둔다 —
      // alt는 그림에 실제로 적힌 글자를 옮기는 것이고, 그림에는 '소스바'로 구워져 있다.
      '<figure class="col-figure col-figure--no-caption">' +
      ...
      // 🔴 가이드 본문에서만 '셀프소스바'로 부른다(2026-07-28 사용자 확정). 태블릿 실제 표기가
      //   '셀프소스바 (1인)'이라, 첫 방문자가 화면에서 찾을 때 같은 말이어야 헤매지 않는다.
      //   사이트의 나머지(레시피 조리 단계·모달 제목·푸터 등 30여 곳)는 '소스바' 그대로 둔다 — 바꾸지 말 것.
      '<p>맑은 탕(물)은 셀프소스바 재료를 넣어 나만의 탕을 만들거나, 국자를 헹구는 용도로 활용할 수 있어요.</p>' +
      // 소스바 인원 선택은 기획초안에서 'STEP 2'였지만, 이것도 태블릿에서 하는 일이라 01 안에 넣는다
      // (2026-07-28 사용자 확정). 별도 '02' 섹션으로 세우면 맨 위 '한눈에 보는 이용 순서' 그림의
      // 02(소스바에서 소스 만들기)와 번호가 어긋나 독자가 헷갈린다.
      // 🔴 소스바 대목의 결정들(2026-07-28 사용자 확정). 되돌리지 말 것.
      //   - 소제목은 중립 서술. 옛 '소스바를 이용할 사람 수만큼 선택해요'는 "일행 전체가 아니라
      //     쓸 사람만 고르면 된다"로 읽히는데, 그게 맞는지 확인되지 않아 뺐다.
      //     소스바 이용이 자율인지 강제인지는 아예 건드리지 않는다.
      //   - **요금은 있다고 확정해서 쓴다.** '붙을 수 있어요'로 흐렸던 것을 되돌린 것 —
      //     불확실한 건 금액이지 요금의 존재가 아니다(기획초안·공개 자료 모두 유료라고 하고
      //     무료라는 자료는 없다). 요금을 미리 알려주는 쪽이 독자에게 손해가 없기도 하다.
      //   - **금액은 쓰지 않는다.** 지점·시기마다 다를 수 있고 어차피 태블릿에 뜬다.
      //     '금액과 기준은 지점에 따라 다를 수 있어요' 같은 부연도 넣지 않는다.
      //   - 요금과 '즐기는 핵심'을 한 문장에 묶는다 — 비용을 인정하자마자 이유를 대야
      //     '그럴 만하다'로 읽힌다. 따로 떼면 요금이 그냥 부담으로만 남는다.
      '<h3>셀프소스바 이용 인원도 선택해요</h3>' +
      // 한 문장에 '셀프소스바'가 두 번 나오면 무거워서 두 번째 주어는 덜어냈다(뜻은 그대로).
      '<p>탕을 골랐다면, 셀프소스바를 이용할 인원수도 함께 선택해요. 별도 요금이 있지만, 하이디라오를 제대로 즐기는 핵심이에요.</p>' +
      '<p>다양한 재료를 원하는 대로 조합해 나만의 소스를 만들 수 있고, ' +
      '같은 탕에 같은 고기를 먹어도 어떤 소스를 곁들이느냐에 따라 맛이 완전히 달라져요.</p>' +
      '<p>무엇을 넣어야 할지 모르겠다면 하딜고고에서 취향에 맞는 레시피를 찾아보세요!</p>' +
      // 재료 주문도 기획초안에선 'STEP 3'이지만 태블릿에서 하는 일이라 01 안에 둔다(소스바 인원과 같은 이유).
      // 기획초안의 '※ 꼭 따라야 하는 순서는 아니에요' 각주는 넣지 않았다 —
      // 바로 위 '처음부터 많이 주문하지 않아도 괜찮아요'가 이미 같은 말을 하고 있어 겹친다.
      '<h3>먹을 재료도 주문해요</h3>' +
      '<p>이제 고기와 채소, 버섯, 두부류 등 탕에 넣어 먹을 재료를 골라요. ' +
      '처음부터 많이 주문하지 않아도 괜찮아요. 먼저 먹어보고 부족한 메뉴를 추가해도 돼요.</p>' +
      // '처음이라면 전골 4가지 맛을 추천해요'와 같은 자리 — 첫 방문자가 가장 막히는 대목에 두는 추천 소제목.
      '<h3>첫 주문이 고민된다면 이렇게 골라보세요</h3>' +
      '<p>고기 + 채소·버섯 + 두부·완자류를 기준으로 고르고, 면이나 사이드 메뉴는 먹어본 뒤 배부름에 따라 추가해보세요.</p>',
  };
